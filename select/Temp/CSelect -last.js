...
(function(window, undefined) {
    ...
    CSelect = {
        ...
        creatset : function(s, sc) {
            var d = this.VWH(), i = this.Scur,crselect = null;
            while(i < sc) {
                crselect = s[i];
                if(crselect.className == "CSelects") {
                    var c = this.getPosition(crselect).y;
                    if(c <= d) {
                        //s[i].style.visibility = "hidden";
                        console.log(i)
                        this.setSelectStyle(crselect, 200, i);
                        this.Scur++;
                    } else {
                        break;
                    }
         ...
         */
        Intation : function() {
            var s = doc.getElementsByTagName("select"), sc = s.length, c = 100, b = 0, sarr = new Array();
            var _this = this;
            for(var i = 0; i<sc; i++){
                if(crselect.className == "CSelects") {
                    sarr[i] = s[i];
